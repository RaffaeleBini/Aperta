...
import type { AggFn, DateGranularity, ShelfField, ShelfName } from "@/lib/charts/types";

const AGG_FUNCTIONS: AggFn[] = ["sum", "avg", "count", "count_distinct", "min", "max"];
const DATE_GRANULARITIES: DateGranularity[] = ["year", "quarter", "month", "day"];

export function ShelfFieldPill({
  shelf,
  shelfField,
  role,
  isTemporal,
  ...
  return (
    ...
    >
      ...
      {role === "measure" && (
        <Select value={shelfField.agg} onValueChange={(v) => onChangeAgg(v as AggFn)}>
      ...
      {role === "dimension" && isTemporal && (
        <Select
          value={shelfField.dateGranularity ?? "none"}
